...
import { personaArbitrary, invalidPersonaArbitrary } from '../arbitraries/persona.arbitraries';
      ...
      fc.assert(
        fc.property(
          ...
          (invalidPersona) => {
            ...
            const hasName = !!invalidPersona.name && invalidPersona.name.length > 0;
            const hasDescription = !!invalidPersona.description && invalidPersona.description.length > 0;
            const hasKnowledgeCategory = !!invalidPersona.knowledgeCategory && invalidPersona.knowledgeCategory.length > 0;
            ...
            const isValid = hasId && hasName && hasDescription && hasKnowledgeCategory && validTemperature;
      ...
      fc.assert(
        fc.property(
          ...
          (validPersona) => {
            ...
            const hasId = validPersona.id && validPersona.id.length > 0 && validPersona.id.length <= 50;
            const hasName = validPersona.name && validPersona.name.length > 0 && validPersona.name.length <= 100;
            const hasDescription = validPersona.description && validPersona.description.length > 0 && validPersona.description.length <= 500;
            const hasKnowledgeCategory = validPersona.knowledgeCategory && validPersona.knowledgeCategory.length > 0 && validPersona.knowledgeCategory.length <= 50;

            // Validate optional fields if present
            const validTemperature = validPersona.temperature === undefined || (validPersona.temperature >= 0 && validPersona.temperature <= 2);
            const validMaxTokens = validPersona.maxTokens === undefined || validPersona.maxTokens > 0;

            const isValid = hasId && hasName && hasDescription && hasKnowledgeCategory && validTemperature && validMaxTokens;
